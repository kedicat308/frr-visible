...
# 不删 $MGMT_NET:遥测采集器(gnmic-frr/exporters/promtail)可能挂在上面删不掉。
# 幂等:存在就复用(子网一致、节点 IP 不与采集器冲突),不存在才建。

echo "== ensure management network $MGMT_NET ($MGMT_SUBNET) =="
docker network inspect "$MGMT_NET" >/dev/null 2>&1 \
